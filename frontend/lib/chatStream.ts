 ...
 *
 * Kept identical to `mobile/lib/chatStream.ts` for the same reason
 * `chatSse.ts` is: two clients that disagree about when a reply ended disagree
 * about the conversation. Pinned by the `streams` section of
 * `contracts/chat-sse.json`, which the phone reads too.
 */
 ...
 *
 * Chunks are bytes, never strings: this `fetch` and the phone's `expo/fetch`
 * both yield `Uint8Array`, and the decoder is fed with `{stream: true}` so a
