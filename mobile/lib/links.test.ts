...
import { isSafeLink, onMarkdownLinkPress } from "./links";
...
describe("onMarkdownLinkPress", () => {
  it("returns exactly true for an allowed link", () => {
    // The library opens the url only on a strict `true`; anything else — a
    // truthy string, undefined — and the tap does nothing.
    expect(onMarkdownLinkPress("https://example.com")).toBe(true);
  });

  it("returns false for a refused one", () => {
    expect(onMarkdownLinkPress("tel:+70000000000")).toBe(false);
  });
});
